<?php
/**
 * \flat\api\response\unauthorized definition
 *
 * PHP version >=7.1
 *
...
class unauthorized extends \flat\api\response {
   ...
   public function __construct($message="",$data=NULL,int $trace_offset=0) {
      if (empty($message)) {
         ...
         if (isset($trace[1+$trace_offset]) && !empty($trace[1+$trace_offset]['class'])) {
            try {
               $this->message = "Request unauthorized by ".(new \ReflectionClass($trace[1+$trace_offset]['class']))->getShortName();
            } catch (\Exception $e) {
               $this->message = "Request unauthorized";
            }
         }
      } else {
      ...
      $this->_set_status(new \flat\api\status\unauthorized());
   }
