...
abstract class tmpl implements \flat\core\controller, \flat\core\resolver\prepared
{
    ...
    public static function display($design, $data = null, $check_only = false)
    {
        ...
        if (is_object($design)) {
            if ($design instanceof \flat\tmpl\resolvable_design) {
                if ($check_only) return;
                static::design_log('resolvable_design object', get_class($design));
                $design->resolvable_design_output(new \flat\tmpl\data((array) $data));
                return;
            }
            if (!$design instanceof \flat\tmpl\output) {
                throw new tmpl\exception\bad_design('design must be a string, object with \flat\tmpl\output interface, or object with \flat\tmpl\resolvable_design interface');
            }
            $output = $design->get_output($data, $check_only);
            if (!is_scalar($output) && !is_null($output)) throw new tmpl\exception\bad_design('\flat\tmpl\output::get_output() return value must be scalar type or null');
         ...
         */
        if (!is_string($design)) throw new tmpl\exception\bad_design('design must be a string or \flat\tmpl\display object' . "instead got: " . gettype($design));
        if (empty($design)) throw new tmpl\exception\bad_design("design cannot be empty");
        ...
        $design_prefix = trim(static::get_design_class_prefix(), '\\');
        $design_suffix = trim(str_replace("flat\\design\\tmpl", "", $design), '\\');
        $design_class = "$design_prefix\\$design_suffix";

        if (class_exists($design_class) && is_a($design_class, \flat\tmpl\resolvable_design::class, true)) {
            if ($check_only) return;
            
            static::design_log('resolvable_design class', $design_class);
            
            $design_class = new $design_class();
            $design_class->resolvable_design_output(new \flat\tmpl\data((array) $data));
         ...
         */
        if (empty($file)) {
            $file_base = trim(str_replace("flat\\design", "", $design), '\\');
            $file_base = rtrim(\flat\core\config::get("design/basedir"), DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . str_replace("\\", DIRECTORY_SEPARATOR, $file_base);
            $file = $file_base . ".php";
        }
    ...
    final public function set_prepared_on()
    {
        if ($this instanceof \flat\tmpl\data_provider) {
            ...
            return $this->_resolve_display($this->get_data());
        }
        if ($this instanceof \flat\tmpl\ignorable) {
            if ($this->is_ignored()) return;
