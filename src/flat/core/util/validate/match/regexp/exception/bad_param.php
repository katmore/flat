 ...
 * 
 * Purpose:
 *    exception for invalid regex given to match validator
 *
 *
 * PHP version >=5.6
 *
...
class bad_param extends \flat\core\util\validate\match\regexp\exception {
   public function __construct($param,$reason="") {
      if (!empty($reason)) $reason = ". reason: $reason";
      parent::__construct("bad param '$param'".$reason);
   }
}